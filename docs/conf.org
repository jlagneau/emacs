...
FIXME: Working in deamon mode currently cause some issues as the initialization of the package is done in the command line, and some package needs to recognize if the frame was opened in a terminal or in a GUI window.
