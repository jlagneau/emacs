...
This is a Literate Emacs Configuration (LEC). The name of the symbols are prefixed with ~lec/~ as a namespace. Symbols starting with ~--~ after the namespace are used internally and not interactive. The goal is to have a really fast editor, as close as possible from vanilla Emacs, with a nice preset of packages on top, for work. Appearance is also a concern and it should look and feel as modern as possible without breaking the already established goals.

FIXME: Working in deamon mode currently cause some issues.
The initialization of the package is done in the command line, and some package needs to recognize if the frame was opened in a terminal or in a GUI window.
...
As per Emacs  >= 28, native compilation can be enabled, resulting in ~*.eln~  files populating a directory at the root of the configuration directory. Redirect eln-cache files into an appropriate directory.
...
Disable a lot of unused functions and features to speed things up, and set up sane defaults for appearance, garbage collection, process output, native JIT compilation, advice redefinition, and warning minimum level.
...
#+begin_src emacs-lisp
  (use-package recentf
    ...
    ;; Exclude `init.el' and `early-init.el' because they are generated automatically
...
#+begin_src emacs-lisp
  (use-package ligature
    :demand t
    :config
    ;; Disable ligatures for merge markers in every possible major modes
    (ligature-set-ligatures 't '("<<<<<<<" "=======" "|||||||" ">>>>>>>"))
    ;; Enable all interesting ligatures in every possible major modes
...
#+begin_src emacs-lisp
  ...
  ;; FIXME: Dashboard navitagor is broken.
  ...
  ;           (,nil "Reload configuration" "Reload Emacs configuration" (lambda (&rest _) (load-file (file-name-concat user-emacs-directory "init.el"))) warning))
...
#+begin_src emacs-lisp
  (use-package dirvish
    ...
    :custom
    (dirvish-attributes '(file-size subtree-state nerd-icons collapse file-time))
    (dired-listing-switches "-l --almost-all --human-readable --group-directories-first")
    (dirvish-side-attributes '(vc-state file-size subtree-state nerd-icons))
